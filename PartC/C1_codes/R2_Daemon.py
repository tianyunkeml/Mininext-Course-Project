	...
	def run(self):
		...
		while True:
			...
			if w_change == True:
				...
				for line in myfile:
					if 'R2-R4' in line:
						...
						rTable.append({'dest': R4_NET,'mask': R4_MASK,'gateway': R4_GW,'dist': int(temp.replace(' ','')),'itf': 'R2-eth1'})
						R2_R4 = int(temp.replace(' ',''))
			myfile.close()		
			time.sleep(period / 2)
			...
			if w_version == w_version_recv:
				rTable = rt_update(rt_recv,R4_GW,rTable,'R2-eth1',R2_R4)
			logFile = open(currentPath + '/logR2.txt','a')
...
if __name__ == '__main__':
	setting = {'APP':'R2_Daemon','PIDFILE':'/home/mininet/myfolder/copy/quagga-ixp/part_c/logs/R2_Daemon.pid','LOG':'/home/mininet/myfolder/copy/quagga-ixp/part_c/logs/R2_Daemon.log'}
	myApp = myDaemon_R2(setting)
	if sys.argv[1] == 'start':
